new Vue({
	...
	methods: {
		...
		initPage:function() {
			...
			data.append("id",1001)
			httpPost("http://localhost:8080/user/test",data).then(resp=>{
				console.log(resp)
			}) */
		    // http://127.0.0.1:80/live/api/home/initPage
			var that = this;
			httpPost("http://127.0.0.1:80/live/api/home/initPage",{}).then(resp=>{
				//登录成功
				if(resp.data.loginStatus==true) {
					that.initInfo=resp.data;
					console.log("initInfo:" + JSON.stringify(that.initInfo))
					that.loginBtnMsg='';
					that.isLogin =true;
				}
			})
