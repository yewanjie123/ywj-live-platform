new Vue({
    ...
    methods: {
        ...
        sendGift: function(giftId) {
            ...
            data.append("type",0);
            //data.append("roomId",57);
            data.append("roomId",getQueryStr("roomId"));
            data.append("receiverId",this.initInfo.anchorId);
            let that = this;
            console.log("封装礼物信息是:" + JSON.stringify(data))
            ...
            .then(resp => {
               ...
               if (isSuccess(resp)) {
                    that.$message.success('送礼成功');
               }else{
                    that.$message.error('送礼失败');
               }
            });  
        },
        ...
        showBankInfoTab:function() {
            ...
            console.log("showBankInfo:" + this.showBankInfo);
        },
        ...
        startSendRedPacket: function() {
                ...
                window.location.href = "http://127.0.0.1:5500/html/red_packet.html";
        ...
        connectImServer: function() {
            /* let url = "ws://localhost:8086/token=1uihjknqs&&userId=10703";
            ...
            .then(resp => {
                if (isSuccess(resp)) {
                    ...
                    let url = "ws://"+that.imServerConfig.wsImServerAddress+"/" + 
                        that.imServerConfig.token+"/"+that.initInfo.userId+"/1001/"+this.roomId;;
                    console.log(url);
        ...
        websocketOnMessage: function(e) { //数据接收
            // console.log('收到数据的数据是:');
            // console.log(e);
            let wsData = JSON.parse(e.data);
            if(wsData.code == 1001) {
                this.startHeartBeatJob();// 发送心跳包
                ...
                if(respData.bizCode==5555) {
                    ...
                    if(!this.startingRedPacket) {
                        ...
                        console.log("totalCount:" + redPacketConfig.totalCount);
                        console.log("configCode:" + redPacketConfig.configCode);
        ...
        startHeartBeatJob: function() {
            ...
            let jsonStr = {"userId": 10703, "appId": 10001};
            let bodyStr = JSON.stringify(jsonStr);
