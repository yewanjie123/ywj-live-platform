...
//        List<HttpCookie> httpCookieList = request.getCookies().get("ywjtk");
//        if (CollectionUtils.isEmpty(httpCookieList)) {
//            LOGGER.error("请求没有检索到ywjtk的cookie，被拦截");
//            return Mono.empty();
//        }
...
//            LOGGER.error("请求的cookie中的ywjtk是空，被拦截");
//            return Mono.empty();
//        }
//        //token获取到之后，调用rpc判断token是否合法，如果合法则把token换取到的userId传递给到下游
...
//    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
//        // 注释所有手动add跨域header代码！！
//        // OPTIONS预检交给 gateway globalcors 自动处理
//        return chain.filter(exchange);
//    }
//
//    @Override
